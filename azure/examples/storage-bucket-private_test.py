...
def test_customer_managed_key():
    """顧客管理鍵は金庫の URI・鍵名・版に分けて渡す"""
    uri = "https://example-kv.vault.azure.net/keys/data/abcdef"
    cfg = _cfg(key_vault_key_uri=uri)
    ...
    assert cfg["account"]["identity"] == {"type": "SystemAssigned"}


def test_key_uri_without_version():
    """版を省いた URI なら keyversion は空文字（最新を追う）"""
    enc = _cfg(key_vault_key_uri="https://example-kv.vault.azure.net/keys/data")[
        "account"
...
def test_key_uri_format_checked():
    ...
    with pytest.raises(ValueError, match="鍵の URI"):
        _cfg(key_vault_key_uri="https://example-kv.vault.azure.net/secrets/data")
