...
- `ValueError`: actions と data_actions が両方空、assignable_scopes が空、スコープが `/subscriptions/` でも管理グループでもない、管理グループが 2 件以上、データプレーンの操作と管理グループの併用、`*` や `Microsoft.Storage/*` のような広すぎるワイルドカード、`PRIVILEGE_ESCALATING_ACTIONS` に当たる操作（ワイルドカードで包んだ形も含む）、actions と not_actions（data 側も同様）の重複
