...
- MySQL 8.0+ は同じ `ROW_NUMBER` が書ける。5.7 には窓関数が無いので `WHERE NOT EXISTS (SELECT 1 FROM t AS u WHERE u.id <=> t.id AND (u.ts < t.ts OR (u.ts = t.ts AND u.pk < t.pk)))` と書く（`pk` は一意な列）。`u.id = t.id` だと `NULL` キーの行が全部残り、`u.ts < t.ts` だけだと `ts` が同じ行が複数残るので、`ROW_NUMBER() = 1` と同じ結果にするには NULL 安全な比較（MySQL は `<=>`）と一意なタイブレークの両方が要る。同じ形を sqlite は `IS`、duckdb は `IS NOT DISTINCT FROM` で書け、`ROW_NUMBER` 版と一致することを確認した
