...
```python
def fanout_topic(topic: str, subscribers: dict[str, dict], *, message_retention_days: int = 1, schema: str | None = None, kms_key_name: str | None = None) -> dict
```
...
- `filter` は指定した購読だけに入る
- `push_endpoint` を指定した購読は `oidc_token` 付きの `push_config` を持つ。HTTPS 以外と、署名するサービスアカウントの無い指定は `ValueError`
