...
- 環境変数への注入はインスタンスの起動時に解決される。同じリビジョンでも、後から起動したインスタンスは新しい値を取る
- `latest` はインスタンスが起動するたびに解決し直される。壊れた版を作ると、以後に起動したインスタンスだけが失敗し、稼働中のものは動き続けるので原因が見えにくい。版の入れ替えを検証してから配りたいなら `pin_versions` で固定する
