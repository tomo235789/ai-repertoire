...
- `None` を渡した削除規則は作られない
- `ValueError`: 未知のストレージクラス、`STANDARD` への移行、日数が 1 未満、移行の順序の逆転、最後の移行以前の削除、作成から最後の階層の最低保存期間より前の削除、残す版の数が負
...
- 階層には最低保存期間がある。Nearline は 30 日、Coldline は 90 日、Archive は 365 日。前の階層での経過も算入されるので、作成からその日数が経っていれば早期削除の料金はかからない。この関数は削除日数を作成からの日数として検査する
